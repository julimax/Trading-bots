...
def execute_connection(symbol='ETH/USDT', timeframe='1m'):
    ...
    #Ema21 = df["close"].ewm(span=25, adjust=False).mean()
    #print(Ema21)
    #plt.plot(Ema21)
...
def balance(estado, ejecution):
    ...
    if estado == "long" and ejecution == "on":           
       in_possession = wallet/df['close'][98]
       wallet = in_possession * df['close'][98]
       ...
       # if in_possession == 0:
        ...
        valor_in_short = wallet - ((in_possession*df['close'][98])-wallet)
