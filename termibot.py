...
def datos():
	...
	marginLong = float(infoTrade[numberLong]['positionAmt'])*float(infoTrade[numberLong]['entryPrice'])/leverage
	marginShort = float(infoTrade[numberShort]['positionAmt'])*float(infoTrade[numberShort]['entryPrice'])/leverage*-1
...
def close_short():
	client.futures_create_order(symbol='ETHUSDT',positionSide='LONG',side='BUY',type='MARKET',quantity=positionAmtShort)
